...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  return (
    <div
      className='w-full bg-white dark:bg-neutral-950 font-ubuntu md:px-10'
      ref={containerRef}
    >
        ...
        {data.map((item, index) => (
          ...
          >
            <div className='sticky flex flex-col lg:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm'>
              ...
              <h3 className='hidden lg:block text-base lg:pl-20 md:text-xl font-bold text-(--color-brown-transparent) dark:text-neutral-500 '>
                {item.title}
              </h3>
            </div>

            <div className='relative sm:pl-21 pl-12 pr-4 lg:pl-4 w-full'>
              <h3 className='lg:hidden hidden sm:block text-xl md:mb-4 mb-10 mt-20 text-left font-bold text-neutral-500 dark:text-neutral-500'>
                {item.title}
              </h3>
              {item.content}{' '}
            </div>
